...
import { updateSTLSize } from '../loaders/stlLoader.js';
...
/**
 * Start interaction - switch to fast render mode if auto-switch is enabled
 */
function startInteraction() {
  ...
  if (!isInteracting) {
    ...
    // Save current mode and switch to fast mode
    const renderModeSelect = document.getElementById("renderMode");
    if (renderModeSelect && renderModeSelect.value === "full") {
      savedRenderMode = "full";
      
      // For STL with many faces, use wireframe; otherwise use solid
      // Check approximate face count to decide
      const svg = document.getElementById("svg");
      const currentFaceCount = svg?.querySelectorAll('[data-face]').length || 0;
      
      if (currentFaceCount > 500) {
        renderModeSelect.value = "wireframe";
        console.log('Auto-switched to WIREFRAME mode (interaction started, >500 faces)');
      } else {
        renderModeSelect.value = "solid";
        console.log('Auto-switched to SOLID mode (interaction started)');
      }
    }
  }
...
/**
 * End interaction - schedule return to original mode after debounce
 */
function endInteraction() {
  ...
  interactionTimeout = setTimeout(() => {
    if (savedRenderMode && isInteracting) {
      const renderModeSelect = document.getElementById("renderMode");
      if (renderModeSelect) {
        console.log('Auto-switching back to FULL mode (interaction ended)');
...
function setupCubeSizeControls(triggerFullUpdate) {
  ...
  const updateCubeSize = (value) => {
    ...
    // Update STL size if in STL mode
    updateSTLSize();
...
function setupMouseControls(triggerFullUpdate) {
  ...
  svgElement.addEventListener("mousedown", (e) => {
    ...
    // Start interaction for auto-switch
    startInteraction();
  ...
  svgElement.addEventListener("mousemove", (e) => {
    ...
    // Continue interaction (resets timeout)
    startInteraction();
  ...
  svgElement.addEventListener("mouseup", () => {
    ...
    // End interaction (schedules return to full mode after debounce)
    endInteraction();
